...
func ScrapeDocsVibra(user, password string, op *db.Operation) ([]models.ContasAPagar, *rod.Page, *rod.Browser, error) {
	inicio := time.Now()
	...
	if err := rod.Try(func() {
		...
		return nil, nil, nil, fmt.Errorf("erro ao navegar: %v", err)
	}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("erro ao obter HTML: %v", err)
	}

	doc, err := goquery.NewDocumentFromReader(strings.NewReader(html))
	if err != nil {
		return nil, nil, nil, fmt.Errorf("erro ao parsear HTML: %v", err)
	}

	// 4. Baixar PDF de boletos
	pdfPath, exists := doc.Find("#divUrlBoleto object").Attr("data")
	if !exists {
		return nil, nil, nil, fmt.Errorf("link do PDF não encontrado")
	}
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("erro ao baixar PDF: %v", err)
	}

	// 5. Dividir PDF em páginas individuais
	err = util.OrdersPDFS()
	if err != nil {
		return nil, nil, nil, fmt.Errorf("erro ao dividir PDF: %v", err)
	}
	...
	return contasPagar, authenticatedPage, browser, nil
}
